...
import { ComingSoonMovieCard } from "@/components/coming-soon-movie-card";
...
export default function ComingSoonPage() {
  ...
  const updateURLParams = (newSearch?: string, newGenre?: string, newSort?: string) => {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      ...
      filterConditions.push(`status eq 'Sắp chiếu'`);
  ...
  return (
    <>
      <div className="container mx-auto px-4 py-8">
        {/* Page Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-2">Phim sắp chiếu</h1>
          <p className="text-gray-400">
            Khám phá các bộ phim sắp ra mắt
          </p>
        </div>
        {/* Filters and Search */}
        <div className="mb-8 space-y-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {sortBy !== "popularity" && (
              <Badge variant="secondary" className="bg-gray-800">
                Sắp xếp: {sortBy === "release_date" ? "Ngày phát hành" : "Tên phim"}
                <button
                  onClick={() => {
                    setSortBy("popularity");
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
            {filteredMovies.map((movie) => (
              <ComingSoonMovieCard
                ...
                releaseDate={
                  movie.releaseDate && typeof movie.releaseDate === "object"
